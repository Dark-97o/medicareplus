...
- **Safety Intervention System:** Monitors user input in real-time for sensitive keywords (suicide, self-harm, etc.). Triggers an immediate, supportive modal with emergency helplines (Vandrevala, iCall, AASRA) and empathetic orientation to prevent crises.
- **Therapist Specialization:** Added a dedicated 'Therapist' track to the platform for specialized mental health support, complementing the existing Psychiatry and Primary Care flows.
